...
public class AiJobProcessor {
    ...
    public AiJobProcessor(AiJobRepository j, DocumentRepository d, StoragePort s, TextExtractor x, AiProvider p, AiResultRepository r, ProcessedEventRepository i, AiEventPublisher e, StructuredOutputValidator v, Clock c) {
        jobs = j;
        docs = d;
        storage = s;
        extractor = x;
    ...
    public void process(UUID eventId, UUID jobId, UUID correlation) {
        ...
        try {
            String raw = switch (job.type()) {
                case QUESTION_GENERATION -> provider.generateQuestions(source, job.requestJson());
                case ANALYSIS -> provider.analyze(source, job.requestJson());
                case CHAT -> provider.chat(source, job.requestJson());
            };
            String valid = job.type() == JobType.QUESTION_GENERATION ? validator.validateQuestions(raw) : validateTextJson(raw);
            String ref = results.save(job.id(), valid);
            job.complete(ref, Instant.now(clock));
            jobs.save(job);
            inbox.record(eventId, "AI_GENERATION_REQUESTED");
    ...
    private void fail(AiJob j, String c, String m, UUID correlation) {
        j.fail(c, m, Instant.now(clock));
        jobs.save(j);
        events.failed(j, correlation);
    }

    private String validateTextJson(String raw) {
        try {
            var n = new tools.jackson.databind.ObjectMapper().readTree(raw);
            if (n == null || !n.isObject()) throw new InvalidAiOutputException("AI result must be JSON object");
            return n.toString();
        } catch (InvalidAiOutputException e) {
            throw e;
        } catch (Exception e) {
