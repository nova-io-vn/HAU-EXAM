...
public class DocumentService {
    public static final Set<String> TYPES = Set.of("text/plain");
    ...
    public DocumentMetadata upload(UUID owner, String name, String type, long size, InputStream data) {
        if (!TYPES.contains(type)) throw new UnsupportedDocumentException("Unsupported content type: " + type);
        if (size <= 0 || size > max) throw new UnsupportedDocumentException("Document size is invalid");
        var f = storage.store(name, data);
        return repo.save(new DocumentMetadata(UUID.randomUUID(), owner, name, type, size, f.key(), f.checksum(), Instant.now(clock)));
    }
