...
import com.aiservice.domain.model.GeneratedQuestion;
...
public class StructuredOutputValidator {
    ...
    public StructuredOutputValidator(ObjectMapper m) {
        mapper = m;
    }
    ...
    public String validateQuestions(String json) {
        try {
            ...
            for (JsonNode q : items) {
                required(q, "question");
                required(q, "correctAnswer");
                required(q, "explanation");
                String difficulty = required(q, "difficulty");
                if (!levels.contains(difficulty)) throw new InvalidAiOutputException("Invalid difficulty");
                JsonNode options = q.get("options");
                if (options == null || !options.isArray() || options.size() < 2)
                    throw new InvalidAiOutputException("At least two options required");
                Set<String> labels = new HashSet<>();
                for (JsonNode o : options) {
                    labels.add(required(o, "label"));
                    required(o, "content");
                }
    ...
    private String required(JsonNode n, String f) {
        JsonNode v = n.get(f);
        if (v == null || !v.isTextual() || v.asText().isBlank()) throw new InvalidAiOutputException(f + " is required");
        return v.asText();
    }
}
