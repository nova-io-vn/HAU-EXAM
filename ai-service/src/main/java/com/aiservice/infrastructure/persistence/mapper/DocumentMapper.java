...
public class DocumentMapper {
    public DocumentEntity toEntity(DocumentMetadata d) {
        ...
        e.id = d.id();
        e.ownerId = d.ownerId();
        e.originalName = d.originalName();
        e.contentType = d.contentType();
        e.size = d.size();
        e.storageKey = d.storageKey();
    ...
    public DocumentMetadata toDomain(DocumentEntity e) {
        return new DocumentMetadata(e.id, e.ownerId, e.originalName, e.contentType, e.size, e.storageKey, e.checksum, e.createdAt);
    }
}
