...
import com.questionservice.infrastructure.config.CloudinaryProperties;
...
public class CloudinaryImageStorageAdapter implements ImageStoragePort {
    ...
    private final Cloudinary cloudinary; private final CloudinaryProperties properties;
    ...
    public StoredImage upload(ImageUploadCommand command, String folder) {
        ...
        try { Map<?, ?> result = cloudinary.uploader().upload(command.bytes(), ObjectUtils.asMap("folder", folder, "resource_type", "image", "use_filename", false, "unique_filename", true)); return new StoredImage(value(result, "url"), value(result, "secure_url"), value(result, "public_id"), value(result, "format"), number(result, "width"), number(result, "height"), numberLong(result, "bytes")); }
        catch (Exception ex) { throw new ImageUploadException("Could not upload image", ex); }
    }
    public void delete(String publicId) { if (publicId == null || publicId.isBlank() || !properties.configured()) return; try { cloudinary.uploader().destroy(publicId, ObjectUtils.asMap("resource_type", "image")); } catch (Exception ex) { log.warn("Cloudinary image cleanup failed for publicId={} type={}", publicId, ex.getClass().getSimpleName()); } }
