...
function AuthBootstrap({children}){const{bootstrapping,authenticated,currentUser}=useAuth();useEffect(()=>{authStore.bootstrapAuth(authApi.refresh)},[]);useEffect(()=>{if(!authenticated||!currentUser?.id)return;let active=true;usersApi.getMe().then(profile=>{if(active)authStore.updateCurrentUser(profile)}).catch(()=>undefined);return()=>{active=false}},[authenticated,currentUser?.id]);return bootstrapping?<Loading label="Đang khôi phục phiên đăng nhập"/>:children}
