export function normalizeNotification(message) {
  const source = message?.payload || message || {};
  return {
    id: source.id || source.notificationId || message?.eventId,
    eventId: message?.eventId || source.eventId,
    type: source.type || message?.eventType || "SYSTEM",
    title: source.title || "Thông báo mới",
...
export function normalizeNotificationPage(result) {
  if (Array.isArray(result))
    return {
      items: result.map(normalizeNotification),
      page: 0,
      totalPages: 1,
      totalElements: result.length,
    };
  return {
    items: (result?.content || result?.items || []).map(normalizeNotification),
