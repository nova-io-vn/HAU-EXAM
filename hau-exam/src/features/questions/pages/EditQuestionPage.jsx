...
export function EditQuestionPage() {
  ...
  async function submit(event) {
    ...
    if (saving || !canEdit(form, auth)) return;
  ...
  if (loading)
    return (
      <div className="question-state">
        <Loading label="Đang tải trình soạn câu hỏi" />
      </div>
    );
  return (
    <section>
      <PageHeader
        ...
        description={
          canEdit(form, auth)
            ...
            : "Trạng thái hiện tại không cho phép chỉnh sửa từ giao diện."
        }
      />
      <QuestionEditor
        form={form}
