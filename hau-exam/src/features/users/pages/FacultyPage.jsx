...
export function FacultyPage() {
  ...
  return (
    <section className="faculty-page">
      <PageHeader
        title="Quản lý Khoa"
        description="Quản lý danh mục Khoa và phạm vi phân quyền trên toàn hệ thống."
        actions={<Button onClick={() => setEditing({})}>+ Thêm Khoa</Button>}
      />
      <div className="admin-kpi-grid faculty-kpis">
        <article className="admin-kpi">
          <small>Tổng Khoa</small>
      ...
      <div className="surface user-table-surface">
        {loading ? (
          <Loading label="Đang tải danh sách Khoa" />
        ) : error ? (
              ...
              columns={[
                {
                  key: "code",
                  header: "Mã Khoa",
                  render: (f) => <span className="mono">{f.code}</span>,
                },
                ...
                },
                {
                  ...
                  header: "Trạng thái",
                  render: (f) => (
                    <StatusBadge status={f.active ? "ACTIVE" : "DISABLED"} />
                  ),
                },
                {
                  key: "actions",
                  ...
                  render: (f) => (
                    <div className="row-actions">
                      <Link
                        ...
                        to={routes.facultyDetail.replace(":id", f.id)}
                      >
                        Xem
                      </Link>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            await facultiesApi.status(f.id, !f.active);
                            load();
                          } catch (e) {
                            setError(e);
                          }
                      ...
                      >
                        {f.active ? "Vô hiệu hóa" : "Kích hoạt"}
                      </Button>
                    </div>
                  ),
                },
      ...
      <Dialog
        ...
        title={editing?.id ? "Sửa Khoa" : "Thêm Khoa đào tạo mới"}
        onClose={() => setEditing(null)}
      >
        <form onSubmit={save} className="editor-section">
          <Input
            name="code"
            label="Mã Khoa"
            required
          ...
          <div className="form-actions">
            ...
            <Button type="submit">Lưu Khoa</Button>
          </div>
        </form>
      </Dialog>
    </section>
