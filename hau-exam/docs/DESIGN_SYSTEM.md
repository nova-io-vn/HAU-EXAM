...
designPhilosophy:
  inspiration:
  - "Apple Human Interface principles: clarity, hierarchy, focus,
    restraint"
...
typography:
  ...
  display:
    ...
    lineHeight: 1.15
  fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'SF
    Pro Text', 'Inter', 'Segoe UI', Roboto, Helvetica, Arial,
    sans-serif"
  headline:
    fontSize: 24px
    fontWeight: 700
...
> **Vị trí đặt file:** `frontend/docs/DESIGN_SYSTEM.md`
...
The HAU Exam Bank Design System is a modern academic workspace built for
lecturers, subject administrators, system administrators, and
AI-assisted content workflows.

The interface should feel calm, precise, modern, and highly usable. The
visual direction is inspired by Apple's emphasis on clarity, hierarchy,
restraint, and content-first interaction, while still supporting the
high information density required by question banks, exam matrices,
review workflows, dashboards, and AI-generated content.

The system must not feel like a marketing landing page. It is a
professional application workspace.

### Key Characteristics

-   **Calm and Focused**: Neutral surfaces, restrained accent colors,
    and strong information hierarchy.
-   **Apple-Inspired Restraint**: Clean typography, generous spacing,
    subtle boundaries, and minimal decorative noise.
-   **Academic Professionalism**: Designed for structured educational
    content and administrative workflows.
-   **High Usability**: Dense information is organized through
    hierarchy, spacing, grouping, and filtering rather than excessive
    decoration.
-   **Consistent Interaction**: Similar actions must look and behave
    consistently across every role and module.
...
The system contains complex academic data, so screens may be
information-dense.
...
The interface uses a neutral Apple-inspired palette with a restrained
blue accent.
...
Accent and semantic colors should normally occupy less than
approximately 10% of the visible application surface.
...
The interface uses a system-first font stack inspired by Apple's product
typography.
...
Do not require proprietary Apple font files to be bundled with the
application.
...
AI features should feel integrated into the product, not like a
...
Never show fake percentage progress if backend does not provide actual
progress.
...
AI and user messages should be visually distinct but not highly
decorative.
...
Important notifications may also exist in the persistent notification
center.
...
If border already defines the surface clearly, do not add a large
shadow.
...
Buttons performing actions should show loading state and prevent
...
Empty states should explain what happened and provide one useful next
action.
...
Do not force desktop table layouts into narrow screens without
...
The system is inspired by Apple's interface philosophy, but must remain
its own product.
...
Subtle translucency may be used only for temporary or fixed navigation
surfaces such as:
...
Frontend visibility must reflect backend authorization, but frontend
...
Frontend must never assume hidden actions are secure; backend remains
authoritative.
...
> A quiet, intelligent, modern academic workspace where the interface
> disappears behind the work.

It should not feel like:

> A colorful dashboard template assembled from unrelated UI kits.

Every visual decision must support clarity, speed, trust, and academic
professionalism.
