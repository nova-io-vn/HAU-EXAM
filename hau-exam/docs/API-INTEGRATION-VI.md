...
### Gán SUBJECT_ADMIN cho Khoa

`PATCH /api/v1/faculties/{facultyId}/subject-admin` với `{ "userId": "..." }` dùng quyền SYSTEM_ADMIN. Danh sách lựa chọn lấy từ User Service, không nhập UUID thủ công. Response Faculty có `lecturerCount` và `subjectAdmins` derived từ User profiles.
