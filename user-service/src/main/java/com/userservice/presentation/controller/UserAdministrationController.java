...
public class UserAdministrationController {
    ...
    @GetMapping public ApiResponse<PageResponse<UserProfileResponse>> list(@AuthenticationPrincipal Jwt jwt,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size,@RequestParam(required=false) String keyword,@RequestParam(required=false) String facultyId,@RequestParam(required=false) Role role,@RequestParam(required=false) com.userservice.domain.model.UserStatus status){var result=useCase.list(actor(jwt),new PageQuery(page,size,keyword,facultyId,role,status,null));return ApiResponse.success(new PageResponse<>(result.content().stream().map(mapper::toResponse).toList(),result.page(),result.size(),result.totalElements(),result.totalPages()));}
    ...
    @PostMapping("/{id}/approve") public ApiResponse<UserProfileResponse> approve(@AuthenticationPrincipal Jwt jwt,@PathVariable UUID id,@Valid @RequestBody(required=false) ApproveUserRequest request,@RequestHeader(value="X-Correlation-Id",required=false) String c){var correlation=correlation(c);return response(request==null?useCase.approve(actor(jwt),id,correlation):useCase.approve(actor(jwt),id,request.facultyId(),request.role(),correlation));}
    ...
    private ApiResponse<UserProfileResponse> response(com.userservice.domain.model.UserProfile u){return ApiResponse.success(mapper.toResponse(u));}
