...
public class RegistrationService implements RegistrationUseCase {
    ...
    public RegistrationService(UserProfileRepository repository, ProcessedEventStore events, Clock clock) {
    ...
    public boolean createFromRegistration(RegistrationCommand c) {
        ...
        if (repository.existsById(c.userId()) || repository.existsByLecturerCode(c.lecturerCode())) {
            events.record(c.eventId(), EVENT_TYPE, Instant.now(clock));
            return false;
        }
        ...
        events.record(c.eventId(), EVENT_TYPE, now);
        return true;
    }
}
