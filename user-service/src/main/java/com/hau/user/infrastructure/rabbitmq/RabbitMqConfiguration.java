...
public class RabbitMqConfiguration {
    ...
    @Bean Queue registrationRetryQueue(@Value("${user.messaging.retry-delay-ms:5000}") int delay){return QueueBuilder.durable(RabbitNames.RETRY_QUEUE).ttl(delay).deadLetterExchange(RabbitNames.AUTH_EXCHANGE).deadLetterRoutingKey(RabbitNames.REGISTRATION_KEY).build();}
